...
class WorkOrderController extends Controller
{
    ...
    public function complete(Request $request, WorkOrder $workOrder)
    {
        ...
        $requestedDisposal = $request->boolean('request_disposal');

        DB::transaction(function () use ($workOrder, $validated, $requestedDisposal) {
            $completedAt = now();
            ...
            if ($workOrder->schedule_id) {
                ...
                if ($schedule) {
                    $schedule->update([
                        'next_due_date' => $completedAt->copy()->addDays($schedule->interval_days)->toDateString(),
                    ]);
                    $this->logActivity($workOrder, "Recurring schedule advanced — next occurrence " . $completedAt->copy()->addDays($schedule->interval_days)->format('M j, Y'));
                }
            }

            // Disposal request — only reachable when condition was set
            // to unserviceable, and only created if the technician
            // actually checked the box, not automatically
            if ($validated['condition'] === 'unserviceable' && $requestedDisposal) {
                AssetDisposal::create([
                    'asset_id'              => $workOrder->asset_id,
                    'reason'                => "Marked unserviceable during {$workOrder->work_order_number}: {$validated['resolution_notes']}",
                    'requested_by'          => auth()->id(),
                ...
                $this->logActivity($workOrder, 'Disposal requested — asset flagged as unserviceable');
            }
